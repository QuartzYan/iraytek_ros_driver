...
enum
{
    ...
    HDISK_RECTYPE_MQTT_NARI         = (1 << 12), //NARI通过MQTT下发snapShot指令
    HDISK_RECTYPE_THRESHOLD         = (1 << 13), //阈值测温报警
    HDISK_RECTYPE_TEMPDIFF          = (1 << 14), //温度对比报警
...
#ifndef USE_HANDLE_32												// Win下64位兼容
    void*               handle;             // handle
#else
    int                 handle;             // handle
#endif
...
typedef struct
{
    int            m_state;        //设置: 0-等待，1-强制终止；获取，详见VSNET_HUMANCALI_STATE_E
    int            m_errcode;      //错误码，仅获取返回, 详见VSNET_HUMANCALI_ERRCODE_E
}VSNET_HUMANCALI_STATE_S;       //人体测温校准状态
...
typedef struct
{
    int                	m_enable;           // function enable
	int					m_typethreshold;	// 0: >m_htempthreshold, 1: <m_ltempthreshold, 2: [m_ltempthreshold, m_htempthreshold], 3: not belong to[m_ltempthreshold, m_htempthreshold]
    float               m_htempthreshold;   // highTempAlarm Threshold 级别0
    float               m_ltempthreshold;   // lowTempAlarm Threshold 级别0
    int                 m_singlehost;       // label of center for uploading alarm signal
    unsigned char       m_record[16];       // relate to recording
    unsigned char       m_out[8];           // relate to output	1:ON, 0：OFF
    unsigned char       m_enpreno[16];      // relate to preset point
    ...
    unsigned char       m_capjpeg[16];      // relate to capturing
    TIMELIST            m_timelist;         // detection time table define ETI_TIME_DAYTYPE
    float               m_high_level[ALARM_LEVEL_NUM];	//级别1~5
    float               m_low_level[ALARM_LEVEL_NUM];	//级别1~5
    int                 m_s32Reserve[3];//保留
...
{
    int                 m_enable;           // function enable
    int                 m_typethreshold;	// 温升类型, 0: 最高温, 1: 最低温, 2: 平均温 
    float               m_thresholdvalue;	// 温升阈值, [1, 100]摄氏度
    int                 m_duration;	        // 温升持续时间, [1, 24]小时
    int                 m_singlehost;       // label of center for uploading alarm signal
    unsigned char       m_record[16];       // relate to recording
    unsigned char       m_out[8];           // relate to output	1:ON, 0：OFF
    unsigned char       m_enpreno[16];      // relate to preset point
...
typedef struct {
    ...
    int     Emissivity;             //辐射率，范围50 ~ 100
    int     AirTemp;                //大气温度，范围-50 ~ 127℃
    int     SkyCond;                //天空条件，范围0 ~ 100
    int     Humidity;               //湿度，范围0 ~ 100
    int     SubjectDistance;        //目标距离，范围0 ~ 2000 米
    int     reserve[4];             //保留
...
#define REGIONTEMPNUMEX 24 // 区域测温扩展为24个

/* 测温区域参数 */
typedef struct
{
    int m_temptype[REGIONTEMPNUMEX];                  //VS_TEMPTYPE
    VSNETREGIONTEMPAREA m_region[REGIONTEMPNUMEX];    //区域测温参数
...
typedef struct
{
    ...
    int     m_sensitive;                 //灵敏度(0 - 5)，数值越大，灵敏度越高
    int     m_bkgd_flush_speed;          //背景更新速度(0 - 4)，数值越大，背景更新越快
...
typedef enum
{
    ANTIFLICKER_MODE_OUTSIDE = 0x0,     //室外模式，关闭抗闪
    ANTIFLICKER_MODE_INSIDE_50HZ = 0x1, //室内50HZ模式
    ANTIFLICKER_MODE_INSIDE_60HZ = 0x2, //室内60HZ模式
...
typedef struct
{
    VSNETIMAGEFUSEOFFSET m_offset[REGIONTEMPNUM];   //偏移范围[-32, 32]
}VSNET_TEMPREGION_OFFSET;

typedef struct
{
    VSNETIMAGEFUSEOFFSET m_offset[REGIONTEMPNUMEX];   //偏移范围[-32, 32]
}VSNET_TEMPREGION_OFFSET_EX;

// 通道类型
typedef enum
{
    VSNET_CHNTYPE_VIS = 0,      //可见光
    VSNET_CHNTYPE_IR,           //红外
    VSNET_CHNTYPE_BUTT
}VSNET_CHNTYPE_E;

// 鼠标测温，坐标[-1, -1]表示禁用鼠标测温功能
typedef struct
{
    int m_s32x;     //x坐标值, [0, 图像宽-1]
    int m_s32y;     //y坐标值, [0, 图像高-1]
    float m_val;    //点温度, 单位:摄氏度
}VSNET_PIXELCOORD_EX;
...
typedef struct
{
    char            m_SIPServerID[32];  //服务器ID, 如34020000002000000001
    char            m_SIPServerIP[64];  //服务器IP, 如192.168.1.12
    char            m_DomainName[32];   //服务器域, 如34020000
    char            m_Password[32];     //密码
    char            m_UserId[32];       //设备ID, 如34020000001320000001
    unsigned short  m_SIPServerPort;    //服务器端口
    unsigned short  m_LocalSipPort;     //设备端口
    unsigned short  m_TransUDP;         //使用UDP, 1:enable, 0:disable
    unsigned short  m_Reserved;         //保留
    char            m_AudioId[32];      //音频通道ID, 如34020000001330000001
    char            m_AlarmId[32];      //报警通道ID, 如34020000001340000001
    unsigned int    m_Expires;          //注册有效期(秒)
    unsigned int    m_MaxHeartBeatTimes;//最大心跳次数, 保留
    unsigned int    m_HeartBeatTime;    //心跳间隔(秒), 保留
    int             m_Unregister;       //注册状态, 0:无, 1:注册, 2:注销
    int             m_EnableGB28181;    //接入使能, 1:enable, 0:disable
    char            m_devname[32];      //设备名称
    char            m_Reser[32];        //保留
...
typedef struct
{
    int             m_enable;           //使能开关 1:enable, 0:disable
}VSNET_ONVIFPARAM_S;

// 融合模式
typedef enum
{
    VSNET_MSXMODE_MSX = 0,     //融合
    VSNET_MSXMODE_BLEND = 1,   //全彩
    VSNET_MSXMODE_MIX = 2,     //高清
    VSNET_MSXMODE_BLEND_MSX = 3,   //全彩融合，保留
    VSNET_MSXMODE_MIX_MSX = 4, //高清融合
    VSNET_MSXMODE_BUTT
}VSNET_MSXMODE_E;
...
typedef struct
{
    int             m_mode;             //融合模式, VSNET_MSXMODE_E
}VSNET_MSXMODE_S;
...
typedef struct 
{
    int             m_encryption;       //加密方式0:不加密1:SSL 2:STARTTLS, VSNET_MAILEXINFO_E
}VSNET_MAILEXINFO_S;
...
typedef struct
{
    int                 m_point_num;                    //多边形顶点数
    VSNET_PIXELCOORD    m_reg[VSNET_POLYGON_POINT_NUM]; //多边形顶点坐标
}VSNET_POLYGONTEMP_INFO_S;      //多边形信息

typedef struct
{
    VSNET_POLYGONTEMP_INFO_S m_region[REGIONTEMPNUM];
}VSNET_POLYGONTEMP_PARAM_S;    //多边形测温

typedef struct
{
    VSNET_POLYGONTEMP_INFO_S m_region[REGIONTEMPNUMEX];
}VSNET_POLYGONTEMP_PARAMEX_S;    //多边形测温

typedef struct
{
    int            m_type;          //应用模式，0:正常, 1:医疗
    int            m_mode;          //校准模式，0:自动, 1:手动
    unsigned short m_x;             //参考区域-x
    unsigned short m_y;             //参考区域-y
    unsigned short m_width;         //参考区域-width
    unsigned short m_height;        //参考区域-height
    float          m_val;           //参考区域的理论温度, 单位C
}VSNET_HUMANCALI_PARAM_S;    //人体测温校准参数

typedef struct
{
    int            m_en;            //开关，0:不使能, 1:使能
    unsigned short m_x;             //区域-x
    unsigned short m_y;             //区域-y
    unsigned short m_width;         //区域-width
    unsigned short m_height;        //区域-height
}VSNET_TEMPSHIELD_S;              //测温屏蔽区域参数

typedef struct
{
    VSNET_TEMPSHIELD_S m_reg[4];    //区域参数
}VSNET_TEMPSHIELD_PARAM_S;       //测温屏蔽参数

typedef struct
{
    float           m_scale;       //缩放比例
    float           m_offset_x;    //水平偏移
    float           m_offset_y;    //垂直偏移
    float           m_reserve[8];  //保留
}VSNET_HUMANCALI_VISMATCHIR_PARAM_S;

typedef struct
{
    short  m_TNormal;   //x100, 单位C
    short  m_THuman;    //x100, 单位C   
}HUMANCALI_TEMPMAP_S;
typedef struct
{
    unsigned char           m_type;         // 0-human, 1-其他          
    unsigned char           m_count;        // < 30 
    unsigned char           m_reserve[2];   // 保留      
    HUMANCALI_TEMPMAP_S     m_map[30];                          
}VSNET_HUMANCALI_TEMPMAP_S;     //人体校准温度映射表
...
typedef struct
{
    int m_mode;     //曝光模式，0 - 自动, 1 - 手动
    int m_exptime;  //曝光时间，VSNET_EXPTIME_E，自动模式下，用作最大曝光时间; 手动模式下，用作手动曝光时间
}VSNET_EXPOSUREPARAM_S;     //曝光参数

typedef struct
{
    int m_temposd_mode; //1<<0: 最高温 1<<1: 最低温 1<<2: 平均温
}VSNET_TEMPOSDMODE_PARAM_S; //温度叠加模式

typedef struct
{
    int m_reg_enable;      //PIP区域使能
    int m_reg_startx;      //PIP区域起始坐标x
    int m_reg_starty;      //PIP区域起始坐标y
    int m_reg_width;       //PIP区域宽
    int m_reg_height;      //PIP区域高
}VSNET_PIP_REGION_PARAM_S;

typedef struct
{
    VSNETTEMPVALUE      m_region_temp[REGIONTEMPNUMEX]; //区域温度信息, 扩展区域数
}VSNETREGIONTTEMPVALUE_EX;

typedef struct
{
    VSNETREGIONTEMP     m_region_temp[REGIONTEMPNUMEX]; //最高温值, 扩展区域数
...
typedef struct
{
    ...
    VSNETREGION         m_min;      //最低温值
    VSNETREGION         m_avg;      //平均温值
    VSNETREGION         m_cen;      //中心点温值
}VSNETREGIONTEMPEX;

typedef struct
{
    VSNETREGIONTEMPEX     m_glb_temp;                   //全局温值
    VSNETREGIONTEMPEX     m_region_temp[REGIONTEMPNUMEX]; //最高温值
